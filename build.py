...
for c in sorted(colors, key=lambda x: [dimensions.index(k) for k in x]):
   if len(c) <= 3:
      ...
      for c2 in itertools.product(*options):
         ...
         for p in permutations(c2):
            ...
            for i, dim in enumerate(tuple(sorted(c2, key=lambda x: dimensions.index(x.strip("-"))))):
               ...
               suppress_dimensions_list = suppress_dimensions_list + f"* {dim.strip('-')}: {suppress}\n"
            ...
            for i, dim in enumerate(tuple(sorted(c2, key=lambda x: dimensions.index(x.strip("-"))))):
               ...
               encourage_dimensions_list = encourage_dimensions_list + f"* {dim.strip('-')}: {encourage}\n"
      ...
      file_path = Path("corruptio_naturae.md")
      file_path.parent.mkdir(parents=True, exist_ok=True)
      file_path.write_text(silent, encoding="utf-8")

      file_path = Path("corruptio_naturae_with_meta.md")
      file_path.parent.mkdir(parents=True, exist_ok=True)
      file_path.write_text(verbose, encoding="utf-8")
